...
export const mutations = {
    ...
    increaseCartItemQty(state, payload) {
        let selectedItem = state.cartItems.find(item => item.id === payload.id);
        if (selectedItem) {
            selectedItem.quantity+=payload.final_increment;
            /* state.total++; */
            state.amount = calculateAmount(state.cartItems);
        }
    },
    ...
    decreaseCartItemQty(state, payload) {
        let selectedItem = state.cartItems.find(item => item.id === payload.id);
        if (selectedItem && selectedItem.quantity > 1) {
            selectedItem.quantity-=payload.final_decrement;
            //state.total--;
            state.amount = calculateAmount(state.cartItems);
        }
    },
...
export const actions = {
    ...
    increaseCartCheckoutItemQuantity({ commit, state }, payload) {
        commit('increaseCartItemQty', payload);
        const cookieParams = {
            total: state.total,
            amount: state.amount,
            cartItems: state.cartItems
    ...
    decreaseCartCheckoutItemQuantity({ commit, state }, payload) {
        commit('decreaseCartItemQty', payload);
        const cookieParams = {
            total: state.total,
            amount: state.amount,
            cartItems: state.cartItems
